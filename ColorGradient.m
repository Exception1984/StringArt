classdef ColorGradient < handle
    ...
    methods
        function obj = ColorGradient(minValue, maxValue)
            obj.minValue = 0.0;
            obj.maxValue = 1.0;
             ...
             obj.span = obj.maxValue - obj.minValue;
             
             obj.SetColorGradientType(GradientType.Jet)
        end
        
        function SetColorGradientType(obj, type)
            switch type
                case GradientType.Jet
                    obj.SetGradientTypeJet();
                case GradientType.Parula
                    obj.SetGradientTypeParula();
                otherwise
                    obj.SetGradientTypeJet();
            end
        end
        
        function SetGradientTypeJet(obj)
            obj.color = [0.0 0.0 0.5 0.0;
                 0.0 0.0 1.0 0.125;
                 0.0 0.5 1.0 0.250;
                 0.0 1.0 1.0 0.375;
                 0.5 1.0 0.5 0.500;
        ...
        function SetGradientTypeParula(obj)
            v = parulaColors();
            n = linspace(0, 1, size(v, 1))';
            obj.color = [v n];
        end
        
        function col = ColorAtValue(obj, val)
            val = (val - obj.minValue) / obj.span;
            
            indexMask = zeros(size(val));
            residualMask = true(size(val));
            
            for i = 1 : size(obj.color, 1)
                indexMask(residualMask) = i;
                residualMask(residualMask == true) = ...
                    ~(val(residualMask) <= obj.color(i, 4) & residualMask(residualMask == true));
            end
            ...
            currRed = red(indexMask);
            currGreen = green(indexMask);
            currBlue = blue(indexMask);
            currVal = v(indexMask);
            
            prevRed = red(max(1, indexMask - 1));
            prevGreen = green(max(1, indexMask - 1));
            prevBlue = blue(max(1, indexMask - 1));
            prevVal = v(max(1, indexMask - 1));
            
            valueDiff = currVal - prevVal;
            
            lambda1 = zeros(size(valueDiff));
            nonZeroInd = valueDiff > 0;
            lambda1(nonZeroInd) = (currVal(nonZeroInd) - val(nonZeroInd)) ./ valueDiff(nonZeroInd);
            lambda2 = 1 - lambda1;
            
            r = lambda1 .* prevRed   + lambda2 .* currRed;
            g = lambda1 .* prevGreen + lambda2 .* currGreen;
            b = lambda1 .* prevBlue  + lambda2 .* currBlue;
